...
import com.qa.opencart.exceptions.ElementsException;
...
public class ElementUtil {
	...
	private void nullCheck(String value) {
		if (value == null) {
			throw new ElementsException("VALUE IS NULL" + value);
		}
	}
	...
	@Step("Entering value in :{0} with value{1} by waiting {2} seconds")
	...
	@Step("Verfying if the element exists :{0}")
	...
	public List<WebElement> waitForVisibilityOfElementsLocated(By locator, int timeout) {
		...
		try {
			...
			return List.of();// returns empty array list
		}
	...
	@Step("Waiting and Fetching the title")
